package com.example.wenjie.mediaplayerdm.util;

/**
 * Created by wen.jie on 2018/1/2.
 */

public class Constants {

    public static String videoUrl = "http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4";

    public static String localVideoUrl = "/storage/emulated/0/ddmsrec.mp4";
    public static String localVideoUrl3 = "/storage/emulated/0/login_sample.mp4";
    public static String localVideoUrl2 = "/storage/emulated/0/yuancheng_gongju.mp4";
    public static String localVideoUrlFind1 = "/storage/emulated/0/elementary_unarmed_chest_male.mp4";

    public static String phiVideoUrl = "http://static1.gotokeep.com/course/public/elementary_unarmed_chest_male.mp4";//徒手胸肌初级训练
    public static String phiVideoUrl2 = "http://static1.gotokeep.com/course/public/buttocks_cultivating.mp4";//翘臀养成
    public static String phiVideoUrl3 = "http://static1.gotokeep.com/course/public/v_shaped_abs_male.mp4";//人鱼线雕刻
    public static String phiVideoUrl4 = "http://static1.gotokeep.com/course/public/elementary_core_male.mp4";//核心功能入门
    public static String phiVideoUrl5 = "http://static1.gotokeep.com/course/public/lower_limb_athletic_ability_male.mp4";


    public static String hao123photoUrl = "https://gss0.bdstatic.com/5bVWsj_p_tVS5dKfpU_Y_D3/res/r/image/2017-09-27/297f5edb1e984613083a2d3cc0c5bb36.png";
    public static String baiduPhotoUrl = "https://www.baidu.com/img/bd_logo1.png";


}
